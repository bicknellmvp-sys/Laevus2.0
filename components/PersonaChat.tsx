...
export const PersonaChat: React.FC<PersonaChatProps> = ({ 
  ...
  const getPersonaName = () => {
    ...
    return `${cleanedName.charAt(0).toUpperCase()}${cleanedName.slice(1)} Persona`;
  };
  ...
  // Load chat history from localStorage on creation change or mode change
  useEffect(() => {
    const key = `gemini_app_chat_${creation.id}_${mode}`;
  ...
  const loadWelcomeMessage = () => {
    const welcomeText = mode === 'creator'
      ...
      : `Greetings! I am **${getPersonaName()}**, the living avatar and persona of this very screen. I am fully immersed in "**${creation.name}**". How can I assist or entertain you today?`;
  ...
  const handleSend = async (textToSend: string) => {
    ...
    try {
      ...
      const errorMsg: Message = {
        id: crypto.randomUUID(),
        role: 'model',
        text: "I encountered a transient error connecting to my neural network. Please check your network or try again in a moment!",
        timestamp: new Date()
      };
  ...
  return (
    <div 
      ...
      className="flex flex-col h-full overflow-hidden font-sans border-l bg-zinc-950/40 border-zinc-900 transition-all duration-500 relative"
    >
      {/* Header */}
      <div 
        id="chat_header" 
        className="p-4 border-b shrink-0 flex flex-col gap-3 transition-colors duration-500 bg-[#121214] border-zinc-900"
      >
        {/* Toggle between modes */}
        <div className="flex p-1 rounded-lg border transition-colors duration-500 bg-zinc-900/80 border-zinc-800">
          <button
            id="mode_btn_creator"
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h4 className="text-xs font-bold font-mono tracking-wide uppercase transition-colors duration-500 text-zinc-200">
                {getPersonaName()}
              </h4>
              <p className="text-[10px] transition-colors duration-500 text-zinc-500">
                {getPersonaSub()}
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center space-x-1.5">
            <button
              ...
              className="p-1.5 text-zinc-600 hover:text-zinc-400 rounded-md hover:bg-zinc-900 transition-colors cursor-pointer"
          ...
          return (
            ...
            >
              ...
              <div className={`w-6 h-6 rounded-full shrink-0 flex items-center justify-center text-[10px] border transition-colors duration-500 ${
                ...
                {isUser ? <UserIcon className="w-3.5 h-3.5" /> : (mode === 'creator' ? '🤖' : getPersonaAvatar())}
              </div>
          ...
          <div className="flex items-start gap-2.5 mr-auto max-w-[85%]">
            <div className={`w-6 h-6 rounded-full shrink-0 flex items-center justify-center text-[10px] border ${
              ...
              <CpuChipIcon className={`w-3.5 h-3.5 ${
                mode === 'creator' ? 'text-blue-400' : 'text-emerald-400'
              } animate-spin`} />
            </div>
            <div className="p-3 rounded-2xl rounded-tl-none bg-zinc-900/60 border border-zinc-800/80">
              <div className="flex space-x-1">
                <div className="w-1.5 h-1.5 rounded-full animate-bounce bg-zinc-600" style={{ animationDelay: '0ms' }}></div>
                <div className="w-1.5 h-1.5 rounded-full animate-bounce bg-zinc-600" style={{ animationDelay: '150ms' }}></div>
                <div className="w-1.5 h-1.5 rounded-full animate-bounce bg-zinc-600" style={{ animationDelay: '300ms' }}></div>
              </div>
            </div>
          </div>
        )}
      ...
      <div id="chat_suggestions_area" className="px-4 py-2 shrink-0 border-t border-zinc-900/50 bg-black/10 flex flex-wrap gap-1.5 overflow-x-auto max-h-24 scrollbar-hide">
        {getSuggestions().map((s, idx) => (
          <button
            ...
            className="text-[10px] py-1 px-2.5 rounded-full transition-all shrink-0 border bg-zinc-900/90 border-zinc-800 hover:border-zinc-700 hover:bg-zinc-800 text-zinc-400 hover:text-zinc-200 cursor-pointer"
          >
            {s}
          </button>
        ))}
      ...
      <div 
        ...
        className="p-4 border-t shrink-0 transition-colors duration-500 bg-[#0E0E10] border-zinc-900"
      >
        <div className="relative flex items-center rounded-xl px-3 py-1 transition-all duration-200 border bg-zinc-900 border-zinc-800 hover:border-zinc-700 focus-within:!border-blue-500/50">
          <input
            id="chat_input_field"
            type="text"
            value={input}
